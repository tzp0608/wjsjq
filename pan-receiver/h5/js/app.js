...
async function renderSubmit() {
  ...
  // 未绑定网盘也可以提交，服务器会直接上传到收集者网盘
  const localBindHint = document.getElementById('local-bind-hint');
  if (!submitterBaiduBound) {
    if (localBindHint) {
      localBindHint.innerHTML = `
        <div style="text-align:center;padding:12px;background:#e6f7ff;border:1px solid #91d5ff;border-radius:8px;margin-bottom:12px;">
          <p style="color:#1890ff;margin-bottom:0;font-size:13px;">您未绑定百度网盘，文件将直接存储到收集者网盘</p>
        </div>
      `;
    }
  } else {
